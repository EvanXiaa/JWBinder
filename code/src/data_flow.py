...
def get_nearest_statement_son(node, answer=None, fun_expr=False):
    """
    ...
    if isinstance(node.parent, _node.Statement)\
            or (fun_expr and isinstance(node.parent, _node.FunctionExpression)):
    ...
    if len(node.statement_dep_parents) > 1:
        logging.warning('Several statement dependencies are joining on the same node %s',
                        node.name)
    return get_nearest_statement_son(node.parent, fun_expr=fun_expr)
...
def statement_scope(node, scopes, id_list, entry):
    ...
    for child_statement_dep in node.statement_dep_children:
        ...
        if child_statement.parent.name in ('IfStatement', 'ConditionalExpression'):
            ...
            # Now we try to analyze no matter the test is
            if_test = None
...
def handle_call_expr(node, scopes, callee, fun_expr=False, tagged_template=False):
    ...
    # Traverse function again
    # number of returns before traverse
    before_return_num = 0 if not function_def.fun_return else len(function_def.fun_return)
    # The overload is too high, we should stop retraverse if last time we didn't get valuable info
    if function_def.retraverse is False or not (isinstance(function_def.last_return, str) or function_def.last_return is None):
        function_def.set_retraverse()  # Sets retraverse property to True
        scopes = function_scope(node=function_def, scopes=scopes, id_list=[])  # function execution imitation
        # number of returns after traverse
        after_return_num = 0 if not function_def.fun_return else len(function_def.fun_return)
        if function_def.fun_return:
            if after_return_num - before_return_num > 2:
                return_value = None
            elif after_return_num - before_return_num == 2:
                value1 = get_node_value(function_def.fun_return[-1], initial_node=node)
                value2 = get_node_value(function_def.fun_return[-2], initial_node=node)
                if value1 == value2:
                    return_value = value1
                elif isinstance(value1, str) and isinstance(value2, str):
                    return_value = "("+value1 + ")" + " or " + "("+value2 + ")"
                else:
                    return_value = None
            else:
                return_value = get_node_value(function_def.fun_return[-1], initial_node=node)
            function_def.last_return = return_value
        ...
        return_value = function_def.last_return
            ...
            for identifier in identifiers:
                for data_dep in identifier.data_dep_parents:
                    if data_dep.extremity.fun is not None:  # Calling a fun that was defined before
                        ...
                        scopes = handle_call_expr(child, scopes=scopes, callee=callee,
                                                  tagged_template=tagged_template)  # very important for cost
                        break
