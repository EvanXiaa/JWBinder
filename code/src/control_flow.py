...
def epsilon_statement_cf(node):
    ...
    exist_children = set()
    for exist_child in node.control_dep_children + node.statement_dep_children:
        exist_children.add(exist_child.extremity)
    for child in node.children:
        if child in exist_children:
            continue
...
def control_flow_update(ast_nodes):
    """
        Enhance the AST by adding statement and control dependencies to each Node.
    ...
    for child in ast_nodes.children:
        if child.name in _node.STATEMENTS:
            continue
        else:
            for grandchild in child.children:
                link_expression(node=grandchild, node_parent=child)
