...
const FILE_TO_READ = "path_to_dir/temp.wat"
const FILE_TO_WRITE = "path_to_dir/module.json"
...
class SetAbstraction extends Abstraction {
	constructor(lvalue, rvalue) {
		...
		this.RValue = rvalue;
		this.vname = null;
		this.Rname = rvalue.variableName;
		this.NewSet = false;
	}
}
...
class BrAbstraction extends Abstraction{
	constructor(step) {
		super('BR');
		this.step = step;
	}
...
class CallAbstraction extends Abstraction {
	constructor(functionName, argsPassed, result) {
		super('CALL');
		this.End = this.Start;
...
class IfAbstraction extends Abstraction {
	constructor(condition, from_br_if = false, depth = 0, step=0) {
		super('IF');
		this.depth = depth
...
var c_num = 0;
var binary_ops = [
	"i32.add","i32.sub","i32.mul","i32.div_s","i32.div_u","i32.rem_s","i32.rem_u","i32.and","i32.or","i32.xor","i32.shl","i32.shr_u","i32.shr_s","i32.rotl","i32.rotr","i32.eq","i32.ne","i32.lt_s","i32.le_s","i32.lt_u","i32.le_u","i32.gt_s","i32.ge_s","i32.gt_u","i32.ge_u","i64.add","i64.sub","i64.mul","i64.div_s","i64.div_u","i64.rem_s","i64.rem_u","i64.and","i64.or","i64.xor","i64.shl","i64.shr_u","i64.shr_s","i64.rotl","i64.rotr","i64.eq","i64.ne","i64.lt_s","i64.le_s","i64.lt_u","i64.le_u","i64.gt_s","i64.ge_s","i64.gt_u","i64.ge_u","f32.add","f32.sub","f32.mul","f32.div","f32.eq","f32.ne","f32.lt","f32.le","f32.gt","f32.ge","f32.min","f32.max","f64.add","f64.sub","f64.mul","f64.div","f64.eq","f64.ne","f64.lt","f64.le","f64.gt","f64.ge","f64.min","f64.max"];
var binIns2op = {"i32.add":"+", "i32.sub":"-", "i32.mul":"*","i32.div_s":"/","i32.div_u":"/","i32.rem_s":"%","i32.rem_u":"%","i32.and":"&","i32.or":"|","i32.xor":"^","i32.shl":"<<","i32.shr_u":">>","i32.shr_s":">>>","i32.eq":"==","i32.ne":"!=","i32.lt_s":"<","i32.le_s":"<=","i32.lt_u":"<","i32.le_u":"<=","i32.gt_s":">","i32.ge_s":">=","i32.gt_u":">","i32.ge_u":">=","i64.add":"+","i64.sub":"-","i64.mul":"*","i64.div_s":"/","i64.div_u":"/","i64.rem_s":"%","i64.rem_u":"%","i64.and":"&","i64.or":"|","i64.xor":"^","i64.shl":"<<","i64.shr_u":">>","i64.shr_s":">>>","i64.eq":"==","i64.ne":"!=","i64.lt_s":"<","i64.le_s":"<=","i64.lt_u":"<","i64.le_u":"<=","i64.gt_s":">","i64.ge_s":">=","i64.gt_u":">","i64.ge_u":">=","f32.add":"+","f32.sub":"-","f32.mul":"*","f32.div":"/","f32.eq":"==","f32.ne":"!=","f32.lt":"<","f32.le":"<=","f32.gt":">","f32.ge":">=","f64.add":"+","f64.sub":"-","f64.mul":"*","f64.div":"/","f64.eq":"==","f64.ne":"!=","f64.lt":"<","f64.le":"<=","f64.gt":">","f64.ge":">="}
var unary_ops = ["i32.clz","i32.ctz","i32.popcnt","i32.eqz","i64.clz","i64.ctz","i64.popcnt","i64.eqz","f32.abs","f32.neg","f32.copysign","f32.ceil","f32.floor","f32.trunc","f32.nearest","f32.sqrt","f64.abs","f64.neg","f64.copysign","f64.ceil","f64.floor","f64.trunc","f64.sqrt","f64.nearest","i32.wrap_i64","i32.trunc_s_f32","i32.trunc_s_f64","i32.trunc_u_f32","i32.trunc_u_f64","i32.reinterpret_f32","i64.extend_s_i32","i64.extend_u_i32","i64.trunc_s_f32","i64.trunc_s_f64","i64.trunc_u_f32","i64.trunc_u_f64","i64.reinterpret_f64","f32.demote_f64","f32.convert_s_i32","f32.convert_s_i64","f32.convert_u_i32","f32.convert_u_i64","f32.reinterpret_i32","f64.promote_f32","f64.convert_s_i32","f64.convert_s_i64","f64.convert_u_i32","f64.convert_u_i64","f64.reinterpret_i64"];

class VariableDeclarationAbstraction extends Abstraction{
	constructor(expressionType, vname=null) {
		super("DECLARE");
		this.End = this.Start;
		this.expressionType = expressionType;
		this.arguments = [];
		this.vname = vname;
		if(expressionType == "load"){
			Simulator.VirtualStates[Simulator.VirtualStates.length - 1].forEach((activeState) => {
				this.vname = "v_"+(v_num++).toString();
				let temp = activeState.VirtualStack.pop();
				this.arguments.push(temp);
				activeState.VirtualStack.push(temp);
			}, this);
		}
		else if (expressionType == "store"){
			//now processed by store
		}
		else if(expressionType == "get_local"){
			//now processed by set
		}
		else if(expressionType == "set_local"){
				//now processed by set
		}
		else if(expressionType == "tee_local"){
			//now processed by set
		}
		else if(expressionType == "get_global"){
		}else if(expressionType == "set_global"){
		}
		else if(expressionType == "const"){
			Simulator.VirtualStates[Simulator.VirtualStates.length - 1].forEach((activeState) => {
				this.vname = "c_"+(c_num++).toString();
				let temp = activeState.VirtualStack.pop();
				this.arguments.push(temp);
				activeState.VirtualStack.push(temp);
			}, this);
		}
		else if(binary_ops.includes(expressionType)){
			Simulator.VirtualStates[Simulator.VirtualStates.length - 1].forEach((activeState) => {
				this.vname = "v_"+(v_num++).toString();
				let by = activeState.VirtualStack.pop();
				let num = activeState.VirtualStack.pop();
				this.arguments.push(num);
				this.arguments.push(by);
				activeState.VirtualStack.push(num);
				activeState.VirtualStack.push(by);
			}, this);
		}
		else if(unary_ops.includes(expressionType)){
			Simulator.VirtualStates[Simulator.VirtualStates.length - 1].forEach((activeState) => {
				this.vname = "v_"+(v_num++).toString();
				let num = activeState.VirtualStack.pop();
				this.arguments.push(num);
...
class VirtualState {
	...
	getLocalVariable(varName) {
		if (!this.LocalVariables[varName]) {
			...
			newVarReference.value.variableName = "local_"+varName.toString();
	...
	getGlobalVariable(varName) {
		if (!this.GlobalVariables[varName]) {
			const newSymbolicValue = new SymbolicValue(varName, lineNumber);
			const newVarReference = new VariableReference(newSymbolicValue, lineNumber);
			newVarReference.value.variableName = "global_"+varName.toString();
	...
	setGlobalVariable(varName, valueToSet) {
		valueToSet.variableName = "global_"+varName;
		if (!this.GlobalVariables[varName]) {
	...
	setLocalVariable(varName, valueToSet) {
		if(Number(varName)>Simulator.ParamNumber-1){
			valueToSet.variableName = "local_"+varName;
		}
		else{
			valueToSet.variableName = "param_"+varName;
		}
		if (!this.LocalVariables[varName]) {
			const newVarReference = new VariableReference(valueToSet, lineNumber);
			this.LocalVariables[varName] = newVarReference;
...
class VirtualStateManager {
	...
	handleConst(value, dataType) {
		...
		const newDecAbstraction = new VariableDeclarationAbstraction("const");
		this.addAbstraction(newDecAbstraction);
		newImmediateValue.variableName = newDecAbstraction.vname;
	}

	handleBinaryOperation(token) {
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState) => {
			let newDecAbsraction = new VariableDeclarationAbstraction(token);
			this.addAbstraction(newDecAbsraction);
			...
			newOperation.variableName = newDecAbsraction.vname
	...
	handleUnaryOperation(token) {
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState) => {
			const newDecAbstraction = new VariableDeclarationAbstraction(token);
			this.addAbstraction(newDecAbstraction);
			const leftValue = activeState.pop();
			const dataType_opCode = token.split('.');
			...
			newOperation = newOperation.execute();
			newOperation.variableName = newDecAbstraction.vname;
			activeState.push(newOperation);
		}, this);
	}

	handleLocal(local_name){
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState) => {
	...
	handleGetLocal(token, nextToken) {
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState) => {
			...
			// 如果local还没声明，先创建一个声明语句，否则直接进栈；
			const if_new_var = !activeState.LocalVariables[varName];
			const valueToPush = activeState.getLocalVariable(varName);
			if(if_new_var&&valueToPush.variableName && valueToPush.variableName.startsWith("local")){
				const newDecAbstraction = new VariableDeclarationAbstraction("get_local");
				this.addAbstraction(newDecAbstraction);
			}
	...
	handleGetGlobal(token, nextToken) {
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState) => {
			...
			const if_new_var = !activeState.GlobalVariables[varName];
			const valueToPush = activeState.getGlobalVariable(varName);
			if(if_new_var&&valueToPush.variableName && valueToPush.variableName.startsWith("global")){
				const newDecAbstraction = new VariableDeclarationAbstraction("get_global", valueToPush.variableName);
				this.addAbstraction(newDecAbstraction);
			}
			activeState.push(valueToPush);
		}, this);
	...
	handleSetLocal(token, nextToken) {
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState, index) => {
			...
			this.CurrentAbstractionList[this.CurrentAbstractionList.length-1].vname = activeState.LocalVariables[varName].value.variableName;
			if(activeState.LocalVariables[varName].previousValues.length==0)
				this.CurrentAbstractionList[this.CurrentAbstractionList.length-1].NewSet = true;
		}, this);
	}
	...
	handleSetGlobal(token, nextToken) {
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState, index) => {
			...
			this.CurrentAbstractionList[this.CurrentAbstractionList.length-1].vname = activeState.GlobalVariables[varName].value.variableName;
			if(activeState.GlobalVariables[varName].previousValues.length==0)
				this.CurrentAbstractionList[this.CurrentAbstractionList.length-1].NewSet = true;
		}, this);
	}
	...
	handleLoad(token, nextToken, secondToken) {
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState) => {
			const newDecAbstraction = new VariableDeclarationAbstraction("load")
			this.addAbstraction(newDecAbstraction)
			...
			newMemoryRef.variableName = newDecAbstraction.vname;
			// binding stack value to variable
	...
	handleIf() {
		...
		// console.log("meet if at line ", lineNumber, " depth= ", this.depth);
	...
	handleBrIf(step) {
		...
		//this.VirtualStates[this.VirtualStates.length - 1][0].push(conditionDetails);
		const fromBrIf = true;
		const newIfAbstraction = new IfAbstraction(conditionDetails, fromBrIf, 0, step);
		newIfAbstraction.end();
		this.addAbstraction(newIfAbstraction);
	}
	handleBr(step){
		// console.log("meet br at line ", lineNumber, " step= ", step, "Dest=", this.depth - 1 -step);
		const newBrAbstraction = new BrAbstraction(step);
		this.addAbstraction(newBrAbstraction);
	}
	handleFor() {
		const newForAbstraction = new ForAbstraction(this.depth);
		// console.log("meet for at line ", lineNumber, " depth= ", this.depth);
	...
	handleBlock() {
		// console.log("meet block at line ", lineNumber, " depth= ", this.depth);
		const newBlockAbstraction = new BlockAbstraction(this.depth);
		...
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState, index) => {
			...
			if (functionType != null && functionType.Params != null) {
				...
				if (functionType.Result != null) {
					resultVar = new SymbolicValue('result'+functionName, lineNumber);
					resultVar.variableName = resultVar.name;
				}
				if (resultVar) {
			...
			if (index == 0) {
				const newCallAbstraction = new CallAbstraction(functionName, namedArgs, resultVar);
	...
	handleCallIndirect(type) {
		this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState, index) => {
			...
			if (index == 0) {
				...
				newCallIndirectAbstraction.index = activeState.VirtualStack.pop()
				activeState.VirtualStack.push(newCallIndirectAbstraction.index);
...
function getFunctionType(functionName) {
	...
	for (const imp of ProgramDetails.Imports) {
		if (imp.ImportedName == functionName  && imp.Type == TOKENS.FUNCTION_TOKEN) {
...
function onLineReadForDetails(line) {
	...
	while (currentTokenIndex < tokens.length) {
		...
		if (token == TOKENS.DATA_TOKEN && tokens[currentTokenIndex + 2] == TOKENS.i32_const) {
			const startOffset = tokens[currentTokenIndex + 3];
		...
		if (token == TOKENS.GLOBAL_TOKEN) {
			let Type = tokens[currentTokenIndex + 2];
			let Mutable = false;
			if(Type == "mut"){
				Type = tokens[currentTokenIndex + 3]
				Mutable = true;
			}
			const ValueTokenIndex = currentTokenIndex + 2 + Mutable;
			if(ValueTokenIndex >= tokens.length)break;
			const Value = tokens.slice(ValueTokenIndex + 1).join(" ");
			const globalValue = new GlobalValue(Type, Value, Mutable);
			ProgramDetails.Globals.push(globalValue);
		}
		...
		else if (token == TOKENS.IMPORT_TOKEN && tokens.includes(TOKENS.GLOBAL_TOKEN)) {
			let globalTokenIndex = tokens.indexOf(TOKENS.GLOBAL_TOKEN);
			const importName = tokens.slice(currentTokenIndex + 1, globalTokenIndex).join(' ');
			const importDetails = new ImportDetails(importName, TOKENS.GLOBAL_TOKEN);
			const importedName = tokens[globalTokenIndex + 1];
			importDetails.ImportedName = importedName;
			ProgramDetails.Imports.push(importDetails);
		}
		else if (token == TOKENS.IMPORT_TOKEN && tokens.includes(TOKENS.TABLE_TOKEN)) {
			let tableTokenIndex = tokens.indexOf(TOKENS.TABLE_TOKEN);
			const importName = tokens.slice(currentTokenIndex + 1, tableTokenIndex).join(' ');
			const importDetails = new ImportDetails(importName, TOKENS.TABLE_TOKEN);
			const importedName = tokens[tableTokenIndex + 1];
			importDetails.ImportedName = importedName;
			ProgramDetails.Imports.push(importDetails);
		}
		else if (token == TOKENS.IMPORT_TOKEN && tokens.includes(TOKENS.MEMORY_TOKEN)) {
			let memoryTokenIndex = tokens.indexOf(TOKENS.MEMORY_TOKEN);
			const importName = tokens.slice(currentTokenIndex + 1, memoryTokenIndex).join(' ');
			const importDetails = new ImportDetails(importName, TOKENS.MEMORY_TOKEN);
			const importedName = tokens[memoryTokenIndex + 1];
			importDetails.ImportedName = importedName;
			ProgramDetails.Imports.push(importDetails);
		}
		...
		if (
			...
			// 		functionDetails.Params.push({ Type: tokens[i] }); // get rid of trailing parenthesis
			// 	}
			//
		...
		while (currentTokenIndex < tokens.length) {
			...
			switch (token) {
				...
				case TOKENS.set_global_alt:
					...
					break;
				case TOKENS.LOCAL_TOKEN:
					{	let i = 1;
						while(['i32','i64','f32','f64'].includes(tokens[currentTokenIndex+i])){
							let local_type = tokens[currentTokenIndex+i];
							Simulator.handleLocal(local_type);
							i++;
						}
						break;
					}
				...
				case TOKENS.br_if:
				{
					let step = tokens[++currentTokenIndex];
					Simulator.handleBrIf(step);}
					break;
				case TOKENS.br:{
					let step = tokens[++currentTokenIndex];
					Simulator.handleBr(step);}
					break;
			}
...
function parseCondition(condition){
	...
	// 	console.log("stay");
	// }//???
	return {"type":"Identifier", "name":condition.variableName};
}
function Abstractions2pdg(abstractions, pdg, label_list){
	for(let abIndex in abstractions){
		let current_abstraction = abstractions[abIndex];
		if (current_abstraction.AbstractionType=="IF"){
			if(current_abstraction.From_br_if){
				label_list.push("IF_"+Global_label++);
				let new_node = {"type":"IfStatement","test":{},"consequent":{"type":"BlockStatement","body":[]}} ;
				let br_node;
				let br_label = label_list[label_list.length - current_abstraction.step -2];
				if(br_label.startsWith("LOOP"))
					br_node = {"type":"ContinueStatement","label":{"type":"Identifier","name":br_label}};
				else if(br_label.startsWith("BLOCK"))
					br_node = {"type":"BreakStatement","label":{"type":"Identifier","name":br_label}};
				new_node.test = parseCondition(current_abstraction.Condition);
				new_node.consequent.body.push(br_node);
				pdg.body.push(new_node);
				label_list.pop();
				Global_label--;
			}
			else{
			label_list.push("IF_"+Global_label++);
			let new_node = {"type":"IfStatement","test":{},"consequent":{"type":"BlockStatement","body":[]},"alternate":{"type":"BlockStatement","body":[]}}
			new_node.test = parseCondition(current_abstraction.Condition);
			Abstractions2pdg(current_abstraction.InnerAbstractions, new_node.consequent, label_list);
			Abstractions2pdg(current_abstraction.ElseAbstractions, new_node.alternate, label_list);
			pdg.body.push(new_node);
			label_list.pop();
			Global_label--;
			}
		}
		else if(current_abstraction.AbstractionType=="BLOCK"){
			label_list.push("BLOCK_"+Global_label++);
			let new_node = {"type":"LabeledStatement","label":{"type":"Identifier","name":label_list[label_list.length-1]},"body":{"type":"ForStatement","init":null,"test":{"type":"Literal","value":1,"raw":"1"},"update":null,"body":{"type":"BlockStatement","body":[]}}}
			Abstractions2pdg(current_abstraction.InnerAbstractions, new_node.body.body, label_list);
			pdg.body.push(new_node)
			label_list.pop();
			Global_label--;
		}
		else if(current_abstraction.AbstractionType=="FOR"){
			label_list.push("LOOP_"+Global_label++);
			let new_node = {"type":"LabeledStatement","label":{"type":"Identifier","name":label_list[label_list.length-1]},"body":{"type":"ForStatement","init":null,"test":{"type":"Literal","value":1,"raw":"1"},"update":null,"body":{"type":"BlockStatement","body":[]}}}
			Abstractions2pdg(current_abstraction.InnerAbstractions, new_node.body.body, label_list);
			pdg.body.push(new_node)
			label_list.pop();
		...
		else if(current_abstraction.AbstractionType=="CALL"){
			let callname = current_abstraction.FunctionName;
			for(let index in ProgramDetails.Imports){
				if(ProgramDetails.Imports[index].Type == "func" && ProgramDetails.Imports[index].ImportedName == callname)
					callname = ProgramDetails.Imports[index].Name.replace(/"/g,"");
			}
			if(current_abstraction.Result){
				let new_node = {"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":current_abstraction.Result.variableName},"init":{"type":"CallExpression","callee":{"type":"Identifier","name":callname},"arguments":[]}}],"kind":"var"};
				for(let argument in current_abstraction.Arguments){
					new_node.declarations[0].init.arguments.push({"type":"Identifier","name":current_abstraction.Arguments[argument].variableName?current_abstraction.Arguments[argument].variableName:"Unknown"});
				}
				pdg.body.push(new_node);
			}
			else{
			let new_node = {'type': "ExpressionStatement", "expression": {"type": "CallExpression", "callee":{type:"Identifier", name:callname}, "arguments":[]}}

			for(let argument in current_abstraction.Arguments){
				new_node.expression.arguments.push({"type":"Identifier","name":current_abstraction.Arguments[argument].variableName?current_abstraction.Arguments[argument].variableName:"Unknown"});
			}
			pdg.body.push(new_node);
			}

		}
		else if(current_abstraction.AbstractionType=="BR"){
			let br_label = label_list[ label_list.length -current_abstraction.step - 1];
			let new_node;
			if(br_label.startsWith("LOOP"))
				new_node = {"type":"ContinueStatement","label":{"type":"Identifier","name":br_label}};
			else if(br_label.startsWith("BLOCK"))
				new_node = {"type":"BreakStatement","label":{"type":"Identifier","name":br_label}};
			pdg.body.push(new_node)
		}
		else if(current_abstraction.AbstractionType=="DECLARE"){
			if(current_abstraction.expressionType=="load"){
				let new_node;
				new_node = {"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":current_abstraction.vname},"init":{"type":"CallExpression","callee":{"type":"Identifier","name":"load"},"arguments":[{"type":"Identifier","name":arguments[0].variableName}]}}],"kind":"var"};
				pdg.body.push(new_node);
			}
			else if(current_abstraction.expressionType=="get_global"){
				let new_node;
				new_node = {"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":current_abstraction.vname},"init":null}],"kind":"var"};
				pdg.body.push(new_node);
			}
			else if(current_abstraction.expressionType=="const"){
				let new_node;
				new_node = {"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":current_abstraction.vname},"init":null}],"kind":"const"};
				if(current_abstraction.arguments[0].value >=0){
					new_node.declarations[0].init = {"type":"Literal","value":current_abstraction.arguments[0].value}
				}
				else{
					new_node.declarations[0].init = {"type":"UnaryExpression","operator":"-","argument":{"type":"Literal","value":-current_abstraction.arguments[0].value},"prefix":true}
				}
				pdg.body.push(new_node);
			}
			else if(binary_ops.includes(current_abstraction.expressionType)){
				let new_node;
				new_node = {"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":current_abstraction.vname},"init":{"type":"BinaryExpression","operator":null,"left":{"type":"Identifier","name":current_abstraction.arguments[0].variableName},"right":{"type":"Identifier","name":current_abstraction.arguments[1].variableName}}}],"kind":"var"};
				if(Object.keys(binIns2op).includes(current_abstraction.expressionType)){
					new_node.declarations[0].init.operator = binIns2op[current_abstraction.expressionType];
				}
				else{
					new_node = {"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":current_abstraction.vname},"init":{"type":"CallExpression","callee":{"type":"Identifier","name":current_abstraction.expressionType.split("_")[0].slice(4)},"arguments":[{"type":"Identifier","name":current_abstraction.arguments[0].variableName},{"type":"Identifier","name":current_abstraction.arguments[1].variableName}]}}],"kind":"var"};
				}
				pdg.body.push(new_node);
			}
			else if(unary_ops.includes(current_abstraction.expressionType)){
				let new_node;
				if(["i32.eqz","i64.eqz"].includes(current_abstraction.expressionType)){
					new_node = {"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":current_abstraction.vname},"init":{"type":"BinaryExpression","operator":"==","left":{"type":"Identifier","name":current_abstraction.arguments[0].variableName},"right":{"type":"Literal","value":0, "raw":"0"}}}],"kind":"var"};
				}
				else{
					new_node = {"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":current_abstraction.vname},"init":{"type":"CallExpression","callee":{"type":"Identifier","name":current_abstraction.expressionType.split("_")[0].slice(4)},"arguments":[{"type":"Identifier","name":current_abstraction.arguments[0].variableName}]}}],"kind":"var"};
				}
				pdg.body.push(new_node);
			}
		}
		else if(current_abstraction.AbstractionType=="SET"){
			let new_node;
			if(current_abstraction.NewSet){
			new_node = {"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":current_abstraction.vname},"init":{"type":"Identifier","name":current_abstraction.Rname}}],"kind":"var"};}
			else{
			new_node = {"type":"ExpressionStatement","expression":{"type":"AssignmentExpression","operator":"=","left":{"type":"Identifier","name":current_abstraction.vname},"right":{"type":"Identifier","name":current_abstraction.Rname}}};
			}
			pdg.body.push(new_node);
		}
		else if(current_abstraction.AbstractionType=="STORE"){
			let new_node;
			new_node = {'type': "ExpressionStatement", "expression": {"type": "CallExpression", "callee":{type:"Identifier", name:"store"}, "arguments":[{type:"Identifier", name:current_abstraction.Value.variableName},{type:"Identifier", name:current_abstraction.MemoryLocation.variableName}]}};
			pdg.body.push(new_node);
		}
		else if(current_abstraction.AbstractionType=="CALL_INDIRECT"){
			console.log("here")
		}
	}
}
async function onLineReadForDetailsEnd() {
	...
	for(let func in ProgramDetails.Functions){
		let this_expression = {type:"ExpressionStatement", expression:{type:"CallExpression", callee:{type:"ArrowFunctionExpression", id:null, params:[], body:{type:"BlockStatement", body:[], range:[], loc:{}}, generator:false, expression:false, async:false, range:[], loc:{}}, arguments:[], range:[], loc:{}}, range:[], loc:{}}
		let this_block = this_expression.expression.callee.body;
		Abstractions2pdg(ProgramDetails.Functions[func].Simulator.Abstractions, this_block, []);
		ProgramDetails.Functions[func].pdg = this_expression;
	}
	const afterFunctionPostProcess = process.memoryUsage();
	const Final_Output = {};
	for(let func in ProgramDetails.Exports){
		let export_name = ProgramDetails.Exports[func].Name.slice(1,-1);
		let export_func = ProgramDetails.Functions[ProgramDetails.Exports[func].ExportSource].pdg;
		Final_Output[export_name] = export_func;
	}

	// add memory information
	let this_memory = {"type":"BlockStatement", "body":[]};
	let variable_name = "module_memory_" + Math.floor(Math.random()*1000).toString();
	this_memory.body.push({"type":"VariableDeclaration","declarations":[{"type":"VariableDeclarator","id":{"type":"Identifier","name":variable_name},"init":null}],"kind":"var"});
	for(let dataSection in ProgramDetails.DataSections){
		let payload = decodeURIComponent(ProgramDetails.DataSections[dataSection].Payload.slice(1, -1));
		let payloads = payload.split("\\00");
		for(let index in payloads){
			if(payloads[index].length>0)
				this_memory.body.push({"type":"ExpressionStatement","expression":{"type":"AssignmentExpression","operator":"+=","left":{"type":"Identifier","name":variable_name},"right":{"type":"Literal","value":payloads[index], "raw":payloads[index]}}})}
	}
	if (this_memory.body.length>1)
		Final_Output["memory"] = this_memory;
	console.log(JSON.stringify(Final_Output));
	fs.writeFileSync(FILE_TO_WRITE, JSON.stringify(Final_Output));
	//DB.quit();
	process.exit();
}
//////////////////////////////////////////////////////////////////
