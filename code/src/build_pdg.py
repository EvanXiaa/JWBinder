...
def get_data_flow(input_file, benchmarks, store_pdgs=None, check_var=False, beautiful_print=False,
    ...
    utility_df.limit_memory(20 * 10 ** 11)  # Limiting the memory usage to 20GB
    ...
    if extended_ast is not None:
        ...
        ast_nodes.id_order = [child.id for child in ast_nodes.children]
...
def find_nearest_parent(node):
    parent = node.parent
    if parent.name in ['VariableDeclarator', 'AssignmentExpression']:
        return parent
    elif parent.name in _node.EPSILON+_node.CONDITIONAL:
        return None
    else:
        return find_nearest_parent(parent)


def simple_taint_analysis_instance(node):
    if node is None:
        print("?")
    children = node.children
    if len(children) > 0:
        for child in children:
            simple_taint_analysis_instance(child)
    if node.name == 'Identifier' and isinstance(node.WASMSource, hooker.wasmObject.WasmInstance):
        parent = node.parent
        ...
        if parent.name == 'VariableDeclarator' or parent.name == 'AssignmentExpression':
            ...
            if nearest_parent is not None:
                ...
                if left_value.name == "Identifier":
                    ...
                    assert 1, "what's wrong here"
    elif node.WASMSource is not None and isinstance(node.WASMSource, hooker.wasmObject.WasmInstance):
