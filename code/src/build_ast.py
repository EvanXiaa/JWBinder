...
def get_extended_ast(input_file, json_path, remove_json=True):
    ...
    try:
        produce_ast = subprocess.run(['/opt/homebrew/bin/node', os.path.join(SRC_PATH, 'parser.js'),
...
def build_json(ast_nodes, dico):
    ...
    for att in ast_nodes.attributes:
        dico[att] = dico[att] if dico.get(att) else ast_nodes.attributes[att]
    return dico


def save_json(ast_nodes):
    """
    ...
    """
    new_ast_nodes = _node.Node('Program')
    num = 0
    for child in ast_nodes.children:
        if child.name == "BlockStatement" and child.children[0].name == "VariableDeclaration" and child.children[0].children[0].children[0].attributes["name"].startswith("module_memory"):
            new_ast_nodes.children.append(child)
            num += 1
        if child.name != "BlockStatement":
            break
    new_children = sorted(ast_nodes.children[num:], key=lambda x: x.id)
    new_ast_nodes.children.extend(new_children)
    data = build_json(new_ast_nodes, dico={})
    code = escodegen.generate(data, {"indent": "\t"})
    return code
